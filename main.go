...
type Portfolio struct {
	...
	StartingCash      float64            `json:"remaining_cash"`
...
func updateUserPLHistoryLoop() {
	for {
		...
		for _, username := range traders {
			...
			point := map[string]interface{}{"time": timestamp, "price": overallProfitOrLoss}
...
func login(w http.ResponseWriter, r *http.Request) {
	...
	if exists && profile.Password == creds.Password {
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"school":  profile.School, // <-- CHANGED: Directly returns the raw school config token (e.g. "AUS")
		})
	} else {
		json.NewEncoder(w).Encode(map[string]interface{}{
			"success": false,
...
func apiUserHistoryMetrics(w http.ResponseWriter, r *http.Request) {
	...
	metricsMap := map[string]interface{}{
		"rank":                        0, 
		"overall_total_p_l":           totalPL,
		"total_transactions":          totalTransactions,
		"total_unsold_shares":         totalUnsoldShares,
		"remaining_cash":              cashBalance,
		"current_stock_holding_value": stockValue,
		"realized_p_l":                realizedPL,
		"unrealized_p_l":              unrealizedPL,
